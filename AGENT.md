...
TanStack Query, chạy ở cổng `5173`, `.env.development` trỏ `VITE_API_BASE_URL` về
`http://localhost:3011`). Lớp API client dùng chung ở `src/lib/api-client.ts` (bọc
khuôn dạng `{data,meta}`/`{error}` của api-conventions.md, tự đăng xuất khi 401 hết
phiên). Đăng nhập lưu JWT + `user.permissions` vào `localStorage`
(`src/lib/auth-context.tsx`); mọi nút hành động trong UI đều gate qua
`hasPermission()` lấy từ quyền backend trả về — không tự suy luận quyền. Đã có trang
cho toàn bộ module đã xong ở backend: Khách hàng, Hợp đồng + Bảng giá (lồng trong chi
tiết hợp đồng vì API không có endpoint list bảng giá), Báo giá, Đơn vận chuyển, Chuyến
(kèm gán nguồn lực, ghép đơn, chuyển trạng thái, và toàn bộ luồng Chứng từ module 7:
upload — bắt buộc `Idempotency-Key`, xác thực, từ chối, khóa, chia sẻ), Xe/Tài
xế/Nhà vận tải, Loại chứng từ. Chưa có: app tài xế, cổng khách hàng/nhà vận tải,
trang cho module 1/8/9/11.
...
| 5. Kế hoạch/điều phối | Gộp vào TripService: ghép/tách đơn vào chuyến (`TripOrderLink`), gán xe/tài xế/NCC, phát lệnh |
...
**Chưa làm**: module 1 (bảng điều hành), module 8 (chi phí/tạm ứng/quyết toán), module
9 (đối soát/hóa đơn/công nợ), module 11 (báo cáo), module 13 (kế toán tổng hợp — chờ
quyết định). Trailer/Container/Seal/Depot (module 10) chưa có model riêng.
